 ...
 */
export function renderLesson(lesson: Lesson, trackName: string): string {
...
function parseProps(tag: string): Record<string, string> {
  ...
  while ((m = stringProps.exec(tag))) props[m[1]] = m[2];
...
function codeBlock(source: string, language: string, label: string): string {
    ...
    if (line.startsWith("```")) {
      ...
      out.push(
        codeBlock(
          body.join("\n"),
          language,
          titleMatch?.[1] ?? (runnable ? `${language} · editable` : language),
        ),
      );
      continue;
    }
...
function renderComponent(name: string, chunk: string): string {
  const props = parseProps(chunk);

  if (name === "Callout") {
    const inner = chunk.replace(/^<Callout[^>]*>/, "").replace(/<\/Callout>\s*$/, "");
    return `<div class="callout"><div class="title">${escapeHtml(
      props.title ?? capitalize(props.type ?? "note"),
    )}</div>${mdxToHtml(inner.trim())}</div>`;
  }

  if (name === "KeyPoints") {
    const inner = chunk.replace(/^<KeyPoints>/, "").replace(/<\/KeyPoints>\s*$/, "");
    return `<div class="box"><p class="label">Worth remembering</p>${mdxToHtml(
      inner.trim(),
    )}</div>`;
  }
  ...
  if (name === "Quiz") {
    ...
    return `<div class="quiz">
      ...
      ${options
        .map(
          (o) =>
            `<button class="opt ${o.correct ? "correct" : "wrong"}">${escapeHtml(
              o.text,
            )}</button><div class="why">${o.correct ? "✓ " : "✗ "}${escapeHtml(o.explain)}</div>`,
        )
        .join("")}
    </div>`;
  }
